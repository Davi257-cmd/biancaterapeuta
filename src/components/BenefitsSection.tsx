...
export function BenefitsSection({ onBooking }: BenefitsSectionProps) {
  return (
    <section className="py-10 md:py-20 px-4 bg-gradient-nude" id="beneficios">
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
          {benefits.map((benefit, index) => (
            <div 
              key={benefit.title}
              className="card-benefit group animate-slide-up p-3 md:p-6 rounded-lg md:rounded-2xl"
              style={{ animationDelay: `${index * 100}ms` }}
            >
              <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-gradient-to-br from-gold/20 to-rose-light flex items-center justify-center mb-2 md:mb-4 group-hover:scale-110 transition-transform duration-300">
                <benefit.icon className="w-5 h-5 md:w-7 md:h-7 text-gold-dark" />
              </div>
